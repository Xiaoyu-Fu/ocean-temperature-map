...
SOURCE_DIR = Path(
    r"D:\Download\其它_DATA2024000003_v4.2_IAPv4.2_IAP_Temperature_gridded_1month_netcdf_Temperature_IAPv4.2_gridded_data_2020_2025"
)
...
def file_for_month(year: int, month: int) -> str:
    ...
    if not path.exists():
        available = sorted(parse_year_month(p) for p in SOURCE_DIR.glob("*.nc"))
        if available:
            first = f"{available[0][0]}-{available[0][1]:02d}"
            last = f"{available[-1][0]}-{available[-1][1]:02d}"
            raise FileNotFoundError(f"未找到 {year}-{month:02d} 数据；当前目录覆盖 {first} 到 {last}。")
        raise FileNotFoundError(f"未找到 NetCDF 数据目录或文件：{SOURCE_DIR}")
    return str(path)
...
class TemperatureHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/profile":
            self.handle_profile(parsed.query)
            return
        self.send_error(404, "Not found")

    def handle_profile(self, query: str) -> None:
        try:
        ...
        except Exception as exc:  # noqa: BLE001 - shown in browser
...
def main() -> None:
    parser = argparse.ArgumentParser(description="IAP temperature profile lookup web app")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8890)
    args = parser.parse_args()
    server = ThreadingHTTPServer((args.host, args.port), TemperatureHandler)
    print(f"Temperature lookup app: http://{args.host}:{args.port}")
